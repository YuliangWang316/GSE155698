...
pbmc.metadata_Tumor<-pbmc.metadata[which(pbmc.metadata$group2 == "T"),]
pbmc.metadata_PBMC<-pbmc.metadata[which(pbmc.metadata$group2 == "P"),]

pbmc_Tumor.data<-pbmc.data[,rownames(pbmc.metadata_Tumor)]
pbmc_PBMC.data<-pbmc.data[,rownames(pbmc.metadata_PBMC)]

pbmc_Tumor <- CreateSeuratObject(counts = pbmc_Tumor.data, project = "IMMUNE_pbmc_Tumor", min.cells = 5,meta.data = pbmc.metadata_Tumor)
pbmc_Tumor$type <- "pbmc_Tumor"
pbmc_Tumor <- subset(pbmc_Tumor, subset = nFeature_RNA > 500)
...
pbmc_PBMC <- CreateSeuratObject(counts = pbmc_PBMC.data, project = "IMMUNE_pbmc_PBMC", min.cells = 5,meta.data = pbmc.metadata_PBMC)
pbmc_PBMC$type <- "pbmc_PBMC"
pbmc_PBMC <- subset(pbmc_PBMC, subset = nFeature_RNA > 500)
...
immune.anchors <- FindIntegrationAnchors(object.list = list(pbmc_Tumor,pbmc_PBMC), dims = 1:20)
...
Treg<-subset(immune.combined,idents = "0")
...
Treg_new<-subset(Treg,idents=c("2"))
...
FeaturePlot(Treg_new,features = "JMJD1C",cols = c("#00008B","#FF69B4"),min.cutoff = "q5",max.cutoff = "q90",pt.size = 1,order = TRUE,reduction = "umap",split.by = "group2")
JMJD1C<-FetchData(Treg_new,vars = "JMJD1C")
JMJD1C_metadata<-data.frame(rownames(Treg_new@meta.data),Treg_new@meta.data$group2)
JMJD1C_new<-cbind(JMJD1C,JMJD1C_metadata[,2])
write.table(JMJD1C_new,file = "GSE155698.txt",sep = "\t")
